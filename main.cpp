...
// Funções de input
void handleMouseInput(int runningGame) {
    if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
        ...
        if (bigLin >= 0 && bigLin < 3 && bigCol >= 0 && bigCol < 3) {
            ...
            if (matrix[boardIndex][smallLin][smallCol] == BoardMark::EMPTY && detect_victory(matrix, winnerMatrix, boardIndex) == 0) {
            ...
            else if(detect_victory(matrix, winnerMatrix, boardIndex) == 2)
                runningGame == false;
        }
    }
}
...
int main(void) {
    ...
    while (!WindowShouldClose() && runningGame == true) {
        BeginDrawing();
        ClearBackground(RAYWHITE);
